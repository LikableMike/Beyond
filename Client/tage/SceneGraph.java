...
/**
 * Tools for building a scene graph tree, and building and applying the
 * associated node controllers.
 * A game application should use the tools here for adding game objects, lights,
 * and node controllers.
 * The game objects and node controllers are stored in ArrayLists.
 * The renderer also uses methods here for applying node controllers at each
 * frame.
 * <p>
 ...
 * </ul>
 * <p>
 * It is important to understand that adding a game object doesn't require
 * calling addGameObject().
 * That function is called by the engine. All that is necessary is to use one of
 * the GameObject constructors.
 * They will call addGameObject(). Similarly it isn't necessary to call
 * buildSkyBox(), that happens automatically.
 * However, adding a Light or a Node Controller does necessitate calling
 * addLight() and addNodeController().
 * <p>
 * Loading a skybox texture into an OpenGL CubeMap using loadCubeMap() is done
 * as follows.
 * The game application needs to supply the name of a folder containing the 6
 * textures.
 ...
 * The game application should store that integer to refer to the cubemap if it
 * wishes to swap
 * between multiple cubemaps. That can be done by calling
 * setActiveSkyBoxTexture() with the integer
 * skybox reference provided as a parameter. All cubemaps should be loaded
...
public class SceneGraph {
	...
	// Called by RenderSystem, should not be called by the game application
	// directly.
	...
	/**
	 * returns a reference to the entire ArrayList of GameObjects - not likely to be
	 * useful to the game application.
	 */
	public ArrayList<GameObject> getGameObjects() {
		return gameObjects;
	}

	protected GameObject getGameObject(int i) {
	...
	protected void updateAllObjectTransforms() {
		root.update();
	}
	...
	public void removeGameObject(GameObject go) {
		...
		} else { // first delete any NodeController references to the game object
			for (int i = 0; i < nodeControllers.size(); i++) {
				nci = nodeControllers.get(i);
				if (nci.hasTarget(go))
					nci.removeTarget(go);
	...
	/**
	 * loads a set of six skybox images into an OpenGL cubemap so that it can be
	 * used in an OpenGL skybox.
	 */
	...
	public void setSkyBoxEnabled(boolean sbe) {
		skyboxEnabled = sbe;
	}

	/** specifies which loaded skybox should be rendered */
	public void setActiveSkyBoxTexture(int tex) {
		activeSkyBoxTexture = tex;
	}

	/** returns an integer reference to the current active skybox */
	public int getActiveSkyBoxTexture() {
		return activeSkyBoxTexture;
	}

	protected GameObject getSkyBoxObject() {
		return skybox;
	}
	...
	protected void setPhysicsEngine(PhysicsEngine peng) {
		pe = peng;
	}
	...
	protected Vector<GameObject> getPhysicsRenderables() {
		return physicsRenderables;
	}

	// prepares the physics objects and shapes for displaying the physics world if
	// enabled
	...
	/**
	 * Adds a box physics object in the physics world, with specified mass, matrix
	 * transform, and size.
	 * <br>
	 * The transform is a 4x4 homogeneous matrix, stored in an array of type double,
	 * which should only contain translation and/or rotation.<br>
	 * The size is specified using a 3-element array of type double, with desired
	 * dimensions in X, Y, and Z.<br>
	 * Also adds a corresponding renderable object if displaying the physics world
	 * has been enabled.
	 */
	...
	/**
	 * Adds a sphere physics object in the physics world, with specified mass,
	 * matrix transform, and radius.
	 * <br>
	 * The transform is a 4x4 homogeneous matrix, stored in an array of type double,
	 * which should only contain translation and/or rotation.<br>
	...
	/**
	 * Adds a cone physics object (pointing upward in +Y direction) in the physics
	 * world, with specified mass, matrix transform, and radius.
	 * <br>
	 * The transform is a 4x4 homogeneous matrix, stored in an array of type double,
	 * which should only contain translation and/or rotation.<br>
	...
	/**
	 * Adds a cone physics object (pointing right in +X direction) in the physics
	 * world, with specified mass, matrix transform, and radius.
	 * <br>
	 * The transform is a 4x4 homogeneous matrix, stored in an array of type double,
	 * which should only contain translation and/or rotation.<br>
	...
	/**
	 * Adds a cone physics object (pointing forward in +Z direction) in the physics
	 * world, with specified mass, matrix transform, and radius.
	 * <br>
	 * The transform is a 4x4 homogeneous matrix, stored in an array of type double,
	 * which should only contain translation and/or rotation.<br>
	...
	/**
	 * Adds a capsule physics object (oriented vertically on Y axis) in the physics
	 * world, with specified mass, matrix transform, radius, and height.
	 * <br>
	 * A capsule has cylindrical sides, but with rounded semi-spherical ends.<br>
	 * The dimensions of a capsule are specified with radius and height, where
	 * radius is for the semi-spheres on each end, and height is the connecting
	 * cylinder.<br>
	...
	/**
	 * Adds a capsule physics object (oriented horizontally on X axis) in the
	 * physics world, with specified mass, matrix transform, radius, and height.
	 * <br>
	 * A capsule has cylindrical sides, but with rounded semi-spherical ends.<br>
	 * The dimensions of a capsule are specified with radius and height, where
	 * radius is for the semi-spheres on each end, and height is the connecting
	...
	/**
	 * Adds a capsule physics object (oriented horizontally on Z axis) in the
	 * physics world, with specified mass, matrix transform, radius, and height.
	 * <br>
	 * A capsule has cylindrical sides, but with rounded semi-spherical ends.<br>
	 * The dimensions of a capsule are specified with radius and height, where
	...
	/**
	 * Adds a cylinder physics object (oriented vertically on Y axis) in the physics
	 * world, with specified mass, matrix transform, radius, and height.
	 * <br>
	 * The dimensions of a cylinder are specified with radius and height.<br>
	 * The transform is a 4x4 homogeneous matrix, stored in an array of type double,
	 * which should only contain translation and/or rotation.<br>
	 * Also adds a corresponding renderable GameObject if displaying the physics
	 * world has been enabled.
	 */
	public PhysicsObject addPhysicsCylinder(float mass, double[] transform, float radius, float height) {
		halfExtents[0] = radius;
		halfExtents[1] = height;
		halfExtents[2] = radius;
	...
	/**
	 * Adds a cylinder physics object (oriented horizontally on X axis) in the
	 * physics world, with specified mass, matrix transform, radius, and height.
	 * <br>
	 * The dimensions of a cylinder are specified with radius and height.<br>
	 * The transform is a 4x4 homogeneous matrix, stored in an array of type double,
	...
	/**
	 * Adds a cylinder physics object (oriented horizontally on Z axis) in the
	 * physics world, with specified mass, matrix transform, radius, and height.
	 * <br>
	 * The dimensions of a cylinder are specified with radius and height.<br>
	 * The transform is a 4x4 homogeneous matrix, stored in an array of type double,
	...
	/**
	 * Adds a a static plane object in the physics world, with specified up vector
	 * and location offset.
	 * <br>
	 * The up vector, usually set to (0,1,0), defines a normal to the plane, and is
	 * opposite the pull of gravity.<br>
	 * The plane_constant, usually set to 0, is an offset (positive or negative) for
	 * moving the plane up and down along its up vector.<br>
	 * Also adds a corresponding renderable object if displaying the physics world
	 * has been enabled.<br>
	 * The renderable object is a 100x100 rectangle, althought the actual physics
	 * object is infinite.
	 */
	...
	/**
	 * removes the specified PhysicsObject from the physics world, and its
	 * associated renderable, if one exists.
	 */
	public void removePhysicsObject(PhysicsObject po) {
		...
		// if it is found in the renderables queue, remove it from the root list and the
		// renderables list
		if (toRemove != null) {
