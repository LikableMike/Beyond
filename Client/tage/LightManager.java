 ...
 * <p>
 * Handling an arbitrary number of lights is complicated because all lights need
 * to be sent to
 * each shader invocation, each time an object is rendered. This is facilitated
 * using an OpenGL SSBO
 * (Shader Storage Buffer Object). Converting the array of light objects to an
 * SSBO requires
 * first converting the array of Light objects to a float array, then to a java
 * direct FloatBuffer,
 ...
 * <p>
 * Although a few accessors have been made public, there shouldn't be any reason
 * for a game
 * application to interact with the LightManager. A game application should
 * instatiate and
 * modify Light objects directly, and insert them into the game via the
 * SceneGraph addLight() method.
 * 
 * @author Scott Gordon
 */
public class LightManager {
	...
	/**
	 * returns a reference to the ith Light - not likely to be useful in the game
	 * application.
	 */
	...
	public int getFieldsPerLight() {
		return fieldsPerLight;
	}

	protected FloatBuffer getLightBuffer() {
		return lightBuf;
	}

	protected float[] getLightArray() {
		return lightArray;
	}

	/**
	 * for engine use only, returns a reference to the SSBO containing the data for
	 * all of the lights
	 */
	public int getLightSSBO() {
		return lightSSBO[0];
	}

	// These functions update the light information in both the FloatArray and the
	// FloatBuffer
