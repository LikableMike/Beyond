...
public class JBulletPhysicsEngine implements PhysicsEngine {
	...
	static {
		PhysicsEngineFactory.registerPhysicsEngine("tage.physics.JBullet.JBulletPhysicsEngine",
				JBulletPhysicsEngine.class);
	}
	...
	public void initSystem() {
		...
		// the maximum size of the collision world. Make sure objects stay within these
		// boundaries
		// TODO: AxisSweep3
		// Don't make the world AABB size too large, it will harm simulation quality and
		// performance
		...
		// dynamicsWorld = new SimpleDynamicsWorld(dispatcher, overlappingPairCache,
		// solver,
		// collisionConfiguration);
	...
	/*
	 * public void createPoint2PointConstraint(PhysicsObject object1, PhysicsObject
	 * object2, double[] aPivot, double[] bPivot){
	 * RigidBody rb1, rb2;
	 * rb1 = ((JBulletPhysicsObject) object1).getRigidBody();
	 * rb2 = ((JBulletPhysicsObject) object2).getRigidBody();
	 * Vector3f aPivotVec = new
	 * Vector3f(JBulletUtils.double_to_float_array(aPivot));
	 * Vector3f bPivotVec = new
	 * Vector3f(JBulletUtils.double_to_float_array(bPivot));
	 * 
	 * Point2PointConstraint constraint = new
	 * Point2PointConstraint(rb1,rb2,aPivotVec,bPivotVec);
	...
	/**
	 * Removes the {@link PhysicsObject} with the specified UID from the physics
	 * world, if it
	...
	public void update(float nanoseconds) {
		if (dynamicsWorld != null) {
			dynamicsWorld.stepSimulation(nanoseconds / 1000f, 4, 1f/60f);// , 4, 1f/60f);
		}
	}
	...
	public PhysicsHingeConstraint addHingeConstraint(int uid, PhysicsObject bodyA, PhysicsObject bodyB, float axisX,
		...
		JBulletHingeConstraint hingeConstraint = new JBulletHingeConstraint(uid, (JBulletPhysicsObject) bodyA,
				(JBulletPhysicsObject) bodyB, axisX, axisY, axisZ);
	...
	public PhysicsBallSocketConstraint addBallSocketConstraint(int uid, PhysicsObject bodyA, PhysicsObject bodyB) {
		JBulletBallSocketConstraint ballSocketConstraint = new JBulletBallSocketConstraint(uid,
				(JBulletPhysicsObject) bodyA, (JBulletPhysicsObject) bodyB);
	...
	@Override
	public Vector<PhysicsObject> getObjects(){
		return this.objects;
	}
