 ...
 * <ul>
 * <li>a link to the associated ObjShape (which includes vertices, texcoords,
 * normals, material)
 ...
 * <li>boolean flags to control propagation of transforms when building world
 * transforms (default true)
 * <li>boolean flags for applying parent rotation/scale to local translation
 * (default false)
 ...
 * its WORLD transforms, which are updated automatically whenever it or its
 * parent
 * are changed. The game application can get the world transforms, but cannot
 * set them directly.
 * <p>
 * There is one "root" GameObject, that is never rendered. It is stored in a
 * static field and
 * static accessors are provided for it. The root node does not have a parent
 * (the parent is null).
 * <p>
 ...
 * <p>
 * A number of other object settings are stored in the attached RenderStates
 * object.
 ...
 * <p>
 * GameObject has several constructors. The no-parameter constructor is
 * protected and only used for the root node.
 * There is a one-parameter constructor that accepts a GameObject - it is for
 * making an empty GameObject.
 * There is also a one-parameter constructor that accepts an ObjShape, that the
 * engine uses for
 * constructing the SkyBox node (the game application shouldn't call this
 * constructor).
 * In all other constructors, the first parameter is for setting the initial
 * parent node.
 * This should be set to Engine.getRoot() in most cases, but may be set to other
 * nodes
 * if building a hierarchical object or system.
 * <p>
 * An application can modify the scenegraph tree by using setParent().
 * Note that this method automatically updates the list of the parent's
 * children.
 * <p>
 * If the game object's shape comes from an OBJ or RKM model that was oriented
 * incorrectly
 * (such as facing sideways or not correctly vertical), the application can set
 * the object's
 * modelOrientationCorrection render state. This is a rotation that is applied
 * when rendering,
...
public class GameObject {
	...
	/**
	 * Builds a GameObject with specified parent node and ObjShape, with texture set
	 * to null.
	 */
	...
	/**
	 * returns a reference to the TextureImage height map associated with this
	 * GameObject - applicable to terrain planes
	 */
	...
	/**
	 * assigns a TextureImage height map to this GameObject - applicable to terrain
	 * planes
	 */
	...
	/** returns a enumerator describing the objects type */
	...
	// It is called automatically by the engine whenever any object transform is
	// modified,
		...
		if (this != root) {
			...
			if (propagateTranslation) {
				...
				if (applyParentRotationToPosition)
					loc.mul(parent.getWorldRotation());
				if (applyParentScaleToPosition)
					loc.mul(parent.getWorldScale());
	...
	// ------------------- accessors for hierarchical systems and hierarchical
	// objects

	/**
	 * applies the parent translation when building this GameObject's world
	 * translation matrix
	 */
	...
	/**
	 * applies the parent rotation when building this GameObject's world rotation
	 * matrix
	 */
	...
	// Note: scaling of a hierarchical object only works if the parent scale factor
	// on all three axes are equal
	...
	/**
	 * returns a boolean that is true if the object applies its parent's rotation to
	 * its position
	 */
	...
	/**
	 * returns a boolean that is true if the object applies its parent's scale to
	 * its position
	 */
	...
	/**
	 * gets the height at (x,z) if this is a terrain plane -- returns 0 if not
	 * terrain, only works if flat on y=0 plane.
	 */
