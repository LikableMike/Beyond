 ...
 * <li>2. set the type of light using the ENUM (POSITIONAL or SPOTLIGHT). The
 * default is POSITIONAL.
 * <li>3. set the desired ADS characteristics using setAmbient, setDiffuse, and
 * setSpecular
 * <li>4. set the desired location using setLocation
 * <li>5. set the desired direction, cutoffAngle, and offAxisExponent if this is
 * a spotlight
 * <li>6. add the light to the game using the addLight() function in SceneGraph
 * </ul>
 * <p>
 * TAGE supports ADS lighting for an unlimited number of positional and spot
 * lights.
 * Global ambient light is also maintained as a static class entity (default
 * RGB=(.3,.3,.3).
...
public class Light {
	...
	/**
	 * returns a reference to the float array containing the global ambient RGB
	 * values
	 */
	public static float[] getGlobalAmbient() {
	...
	protected static void setEngine(Engine e) {
		engine = e;
	}
	// ---------------------------------
	...
	/**
	 * The game application should call this constructor to create a Light object,
	 * then set its characteristics with the accessors.
	 */
	public Light() {
	}
	...
	// The game application should not try to make use of this index, and must not
	// change it.
	...
	public void setLocation(Vector3f l) {
		...
		if (lm.getLightArray() != null)
			lm.updateLightLocation(index, location[0], location[1], location[2]);
	}
	...
	public void setAmbient(float r, float g, float b) {
		...
		LightManager lm = engine.getLightManager();
		if (lm.getLightArray() != null)
			lm.updateLightAmbient(index, ambient[0], ambient[1], ambient[2]);
	}
	...
	public void setDiffuse(float r, float g, float b) {
		...
		LightManager lm = engine.getLightManager();
		if (lm.getLightArray() != null)
			lm.updateLightDiffuse(index, diffuse[0], diffuse[1], diffuse[2]);
	}
	...
	public void setSpecular(float r, float g, float b) {
		...
		LightManager lm = engine.getLightManager();
		if (lm.getLightArray() != null)
			lm.updateLightSpecular(index, specular[0], specular[1], specular[2]);
	}
	...
	public void setConstantAttenuation(float ca) {
		...
		LightManager lm = engine.getLightManager();
		if (lm.getLightArray() != null)
			lm.updateConstantAttenuation(index, constantAttenuation);
	}
	...
	public void setLinearAttenuation(float la) {
		...
		LightManager lm = engine.getLightManager();
		if (lm.getLightArray() != null)
			lm.updateLinearAttenuation(index, linearAttenuation);
	}
	...
	public void setQuadraticAttenuation(float qa) {
		...
		LightManager lm = engine.getLightManager();
		if (lm.getLightArray() != null)
			lm.updateQuadraticAttenuation(index, quadraticAttenuation);
	}
	...
	public void setDirection(Vector3f d) {
		...
		LightManager lm = engine.getLightManager();
		if (lm.getLightArray() != null)
			lm.updateDirection(index, d.x(), d.y(), d.z());
	}
	...
	public void setCutoffAngle(float coa) {
		...
		LightManager lm = engine.getLightManager();
		if (lm.getLightArray() != null)
			lm.updateCutoffAngle(index, cutoffAngle);
	}
	...
	public void setOffAxisExponent(float oae) {
		...
		LightManager lm = engine.getLightManager();
		if (lm.getLightArray() != null)
			lm.updateOffAxisExponent(index, offAxisExponent);
	}
	...
	/**
	 * gets a reference to the float array containing this light's ambient RGB
	 * characteristic
	 */
	...
	/**
	 * gets a reference to the float array containing this light's diffuse RGB
	 * characteristic
	 */
	...
	/**
	 * gets a reference to the float array containing this light's specular RGB
	 * characteristic
	 */
	...
	/**
	 * gets a reference to the float array containing this light's world location
	 * (x,y,z)
	 */
	...
	public float getOffAxisExponent() {
		return offAxisExponent;
	}
}