 ...
 * Includes implementations of methods for keeping track of objects being
 * controlled.
 * Also includes retrievable values for total and incremental elapsed time,
 * and for enabling/disabling the controller.
 * <p>
 * When building a custom node controller class, it should extend
 * NodeController, and
 * implement the method apply() that performs the desired functionality on each
 * attached object.
 * 
 * @author Scott Gordon
 */
public abstract class NodeController {
	...
	/**
	 * Causes the controller to start functioning (starts calling "apply" at each
	 * frame).
	 */
	public void enable() {
		enabled = true;
	...
	/**
	 * returns a boolean that is true if the specified GameObject is in this node
	 * controller's target list
	 */
	public boolean hasTarget(GameObject go) {
	...
	/**
	 * Adds the specified GameObject to the list of GameObjects this controller
	 * affects when enabled.
	 */
	...
	/**
	 * Removes the specified GameObject from the list of GameObjects this controller
	 * affects when enabled.
	 */
	...
	/**
	 * Returns the elapsed time since the last time the controller was applied (if
	 * enabled) - usually means since the last frame.
	 */
	...
	// This function is called by SceneGraph, and should not be called from the game
	// application.
	...
	/**
	 * A node controller should override this function to specify what the
	 * controller does to its target object(s).
	 */
