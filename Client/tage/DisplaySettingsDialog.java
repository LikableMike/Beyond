 ...
 * queries the device to obtain the supported display modes, and provides a GUI
 * which
 ...
 * obtaining the selected values, including DisplayMode (width, height,
 * bit-depth,
 * and refresh rate), and whether or not to use FSEM (Full Screen Exclusive
 * Mode).
 * <P>
 * After creating a DisplaySettingsDialog, call showIt() to make the dialog
 * visible.
 * The visible dialog is modal (meaning the invoking program cannot continue
 * until the dialog
 * is dismissed). Hitting either "OK" or "Cancel" on the dialog dismisses
 * (hides) the
 * dialog, but does not delete it; the invoking code can make it visible again
 * by calling
 * showIt() again.
 * <P>
...
public class DisplaySettingsDialog extends JDialog implements ActionListener {
	...
	private JRadioButton homeTeamButton, awayTeamButton;
	...
	private void doMyLayout() {
		...
		// add a left panel with a radio button group for selecting full or windowed
		// screen mode
		JPanel screenPanel = new JPanel();
		screenPanel.setBorder(new TitledBorder("Screen Mode:  "));
		Box screenButtonBox = new Box(BoxLayout.Y_AXIS);
		Box teamButtonBox = new Box(BoxLayout.Y_AXIS);

		windowedModeRadioButton = new JRadioButton("Windowed", true);
		fullScreenModeRadioButton = new JRadioButton("Full Screen", false);

		screenPanel.setBorder(new TitledBorder("Choose Team:  "));
		homeTeamButton = new JRadioButton("Home", false);
		awayTeamButton = new JRadioButton("Away", false);
		...
		teamButtonGroup.add(homeTeamButton);
		teamButtonGroup.add(awayTeamButton);
		screenButtonBox.add(windowedModeRadioButton);
		screenButtonBox.add(fullScreenModeRadioButton);
		teamButtonBox.add(homeTeamButton);
		teamButtonBox.add(awayTeamButton);

		screenPanel.add(screenButtonBox);
		screenPanel.add(teamButtonBox);
	...
	public void showIt() {
		...
		if (homeTeam) {
			homeTeamButton.doClick();
		} else {
			awayTeamButton.doClick();
		}
	...
	/**
	 * Parses the given modeString to extract the width, height, depth, and refresh
	 * rate
	 * of the mode defined by the string, and returns a DisplayMode object
	 * containing
	 * those values.
	 */
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand() == "OK") {
			...
			homeTeam = homeTeamButton.isSelected();
		}
