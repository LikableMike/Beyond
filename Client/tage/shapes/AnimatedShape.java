...
/**
 * AnimatedShape contains functions for loading animated models, building
 * animated shapes and
 * their skeletons, and applying animations to the models at runtime.
 * It is based on the skeletal mesh classes and functions by Luis Gutierrez
 * (2017) for RAGE.
 * <p>
 ...
 * </ul>
 * <p>
 * Note that the only animated models supported are those exported using the
 * Blender RAGE exporters.
 * 
...
public final class AnimatedShape extends ObjShape {
	...
	// These matrices are multiplied by the vertices to yield their skinned
	// model-space locations.
	...
	/**
	 * Specifies filenames for the model (with extension "rkm") and the skeleton
	 * (with extension "rks").
	 */
	public AnimatedShape(String meshPath, String skelPath) {
		...
		try {
			...
			if ((line = br.readLine()) != null) { // Split this tab-delimited line into an array
	...
	/**
	 * Specifies a string name for an animation, and the file containing the
	 * animation (with extension "rka").
	 */
	...
	public Animation getAnimation(String s) {
		return animationsList.get(s);
	}

	/**
	 * Returns the number of animations ("rka" files) that have been loaded for this
	 * shape.
	 */
	...
	protected HashMap<String, Animation> getAllAnimations() {
	...
	/**
	 * The game application should call this function at each frame (i.e., in
	 * update()) for each animated shape.
	 */
	...
	// Returns the ith bone's local space to model space (when all bones are in rest
	// pose)
	...
	protected tage.rml.Matrix4 getBoneRestTransformRel2Parent(int i) {
		...
		// 3) Then apply parent bone's length in vertical direction (is the y-axis
		// correct?)
	...
	// Returns the ith bone's current local transform for the current animation
	// frame
	...
	// Returns the Quaternion's axis, if is not the identity quaternion, else
	// (1,0,0)
	// This is safer than calling q.axis(), as q.axis() ALWAYS attempts to normalize
	// the axis vector.
	// However, if the Quaternion is the identity quaternion (1,0,0,0), it will
	// attempt to normalize a zero-vector
	// Throwing an arithmetic exception.
	// If an arithmetic exception is thrown, I can return an arbitrary axis (namely,
	// (1,0,0))
	...
	// PINGPONG: plays the animation backwards back and forth endTypeCount number of
	// times

	/**
	 * Play the specified animation at the specified speed, EndType, and number of
	 * times.
	 * <br>
	 * speed == 1.0 for regular, larger for faster, fractional for slower, and
	 ...
	 * <li>LOOP: starts the animation from the beginning endTypeCount number of
	 * times
	 * <li>PINGPONG: plays the animation backwards back and forth endTypeCount
	 * number of times
	 * </ul>
	 */
	...
	/** for engine use only. */
	public tage.rml.Matrix4[] getPoseSkinMatrices() {
		return curSkinMatrices;
	}

	/** for engine use only. */
	public tage.rml.Matrix3[] getPoseSkinMatricesIT() {
		return curSkinMatricesIT;
	}
